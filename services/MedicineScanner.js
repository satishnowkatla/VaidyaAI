...
export const scanMedicineImage = async (base64Image) => {
  try {
    const compressedBase64 = base64Image.substring(0, 900000);
    ...
    console.log("OCR result:", JSON.stringify(result).substring(0, 300));
    ...
    const extractedName = extractMedicineName(text);
...
const normalizeScanTextForMatch = (text) =>
  text
    .toLowerCase()
    .replace(/[^a-z0-9& ]/g, " ")
    .replace(
...
const findKnownMedicineLine = (line) => {
  const normalizedLine = normalizeScanTextForMatch(line);
  const keys = [
    ...new Set([
      ...Object.keys(LOCAL_MEDICINE_DB),
      ...Object.keys(FDA_SYNONYMS),
    ]),
  ];
...
const normalizeForFda = (medicineName) => {
  ...
  const matchedSynonym = Object.keys(FDA_SYNONYMS).find((key) =>
    searchText.includes(key),
  );
  if (matchedSynonym) return FDA_SYNONYMS[matchedSynonym];
...
export const searchMedicineInfo = async (medicineName) => {
  try {
    const cleanName = normalizeForFda(medicineName);

    // Check local database first
    if (LOCAL_MEDICINE_DB[cleanName]) {
      console.log("Found in local DB:", cleanName);
      return {
        ...LOCAL_MEDICINE_DB[cleanName],
        name: medicineName, // Keep original scanned name
      };
    }

    const queries = buildQueries(cleanName);
    console.log("Searching FDA for:", cleanName, queries);
    ...
    for (const query of queries) {
      try {
        ...
        if (!response.ok) {
          console.log("FDA search failed for query:", query, response.status);
          continue;
        }
        ...
        console.log("FDA query error for", query, error.message);
      }
    }
